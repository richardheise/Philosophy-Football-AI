...
int test_philosophers_right(game_t *game, char *temp_field){
    ...
    for (int j = game->ball_pos-2; j >= 0; j--) {
        ...
        if(temp_field_adv[j] != 'f'){
            ...
            h = right_heuristic(temp_field_adv, game->field_size, game->ball_pos);
...
int test_kick_right(game_t *game, char *field){
    ...
    if( right_heuristic(field, game->field_size, game->ball_pos) == 1 ){
        if(game->my_side == 'd')
            return 34000;
        return 1;
    }
    ...
    for (int i = game->ball_pos+1; i < game->field_size; i++) {
...
char* search_best_play_right (game_t *game) {
    ...
    for (int i = game->ball_pos; i < game->field_size; i++) {
        ...
        if (temp_field[i] != 'f') {
            ...
            h = test_philosophers_right(game, temp_field);
